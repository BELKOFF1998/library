...
COMMENT ON COLUMN public.record.data IS 'время когда взяли книги';
...
COMMENT ON COLUMN public.record.id_visitor IS 'ссылка на данные посетителя';
...
COMMENT ON COLUMN public.record.id_employee IS 'ссылка на данные работника библиотека который оформил книги';
...
ALTER TABLE public.record ALTER COLUMN id_record ADD GENERATED ALWAYS AS IDENTITY (
...
COMMENT ON TABLE public.person IS 'таблица персональных данных посетителей и сотрудников';
...
COMMENT ON COLUMN public.person.id_pasport IS 'ссылка на запись с паспортными данными';
...
COMMENT ON COLUMN public.person.birth_data IS 'Дата рождения';
...
ALTER TABLE ONLY public.book_list
    ADD CONSTRAINT book_list_entry_fk FOREIGN KEY (id_entry) REFERENCES public.record(id_record);
...
ALTER TABLE ONLY public.employee
    ADD CONSTRAINT employee_person_fk FOREIGN KEY (id_employee) REFERENCES public.person(id_person);
...
ALTER TABLE ONLY public.person
    ADD CONSTRAINT person_pasport_details_fk FOREIGN KEY (id_pasport) REFERENCES public.pasport_details(id_pasport);
...
ALTER TABLE ONLY public.record
    ADD CONSTRAINT record_employee_fk FOREIGN KEY (id_employee) REFERENCES public.employee(id_employee);
...
ALTER TABLE ONLY public.record
    ADD CONSTRAINT record_visitor_fk FOREIGN KEY (id_visitor) REFERENCES public.visitor(id_visitor);
...
ALTER TABLE ONLY public.visitor
    ADD CONSTRAINT visitor_person_fk FOREIGN KEY (id_visitor) REFERENCES public.person(id_person);
...
GRANT SELECT,INSERT,UPDATE ON TABLE public.record TO employee;
...
GRANT SELECT(id_record) ON TABLE public.record TO guest;
GRANT SELECT(id_record) ON TABLE public.record TO employee;
...
GRANT SELECT,INSERT,UPDATE ON TABLE public.person TO employee;
GRANT SELECT ON TABLE public.person TO guest;
